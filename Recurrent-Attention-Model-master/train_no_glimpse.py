...
from model import RecurrentAttentionModel, NoGlimpseModel
import logging
import numpy as np
...
tf.app.flags.DEFINE_integer("num_glimpses", 0, "Number of glimpses.")
...
ram = NoGlimpseModel(img_size=224,  # MNIST: 28 * 28
...
with tf.Session() as sess:
    ...
    for step in xrange(FLAGS.num_steps):
        ...
        output_feed = [ram.train_op, ram.loss, ram.xent, ram.learning_rate]
        _, loss, xent, learning_rate = sess.run(output_feed, feed_dict={ram.img_ph: images,
                                                                        ram.lbl_ph: labels
                                                                                  })
        if step and step % 100 == 0:
            logging.info(
                'step {}: lr = {:3.6f}\tloss = {:3.4f}\txent = {:3.4f}'.format(step, learning_rate, loss, xent))
