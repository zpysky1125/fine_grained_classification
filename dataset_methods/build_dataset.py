...
with tf.Session() as sess:
    ...
    for epoch in range(100):
        ...
        for i in range(total_batch):
            ...
            _, batch_loss = sess.run([train, loss], feed_dict={images: batch_images, labels: batch_labels, train_mode: True})
            if i % 20 == 0:
                ...
                print ("Training Accuracy: {}".format(accuracy.eval(feed_dict={images: batch_images, labels: batch_labels, train_mode: True})))
        ...
        for i in range(len(valid_labels) / valid_batch_size):
            batch_images, batch_labels = sess.run([next_images, next_labels])
            valid_batch_correct_num, valid_batch_loss = sess.run([num_correct_preds, loss], feed_dict={images: batch_images, labels: batch_labels, train_mode: True})
