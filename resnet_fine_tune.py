...
class ResNetFineTuneModel:
    ...
    def model(self):
        ...
        self.probs = tf.reduce_mean(tf.nn.softmax(self.logits) * tf.one_hot(self.labels, 200), axis=-1)
        ...
        self.train_op = tf.train.AdamOptimizer(learning_rate=self.learning_rate).apply_gradients(zip(gradients, params))
    ...
    def run(self):
        ...
        train_writer = tf.summary.FileWriter('log/resnet_fine_tune/' + str(self.learning_rate) + '_' + str(cur_time) + '/board/train', sess.graph)
        test_writer = tf.summary.FileWriter('log/resnet_fine_tune/' + str(self.learning_rate) + '_' + str(cur_time) + '/board/test', sess.graph)
        ...
        for epoch in range(self.train_epoch):
            ...
            saver.save(self.sess, 'log/resnet_fine_tune/' + str(self.learning_rate) + '_' + str(cur_time) + '/tmp/model.ckpt', global_step=epoch)
